    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            На основе сгенерированной онтологии документы автоматически разбиваются и отправляются в Zep для построения графа знаний, извлечения сущностей и связей, формирования временной памяти и обзоров сообществ
          </p>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        <div class="card-header">
          ...
          <div class="step-status">
            <span v-if="currentPhase >= 2" class="badge accent">Выполняется</span>
          </div>
        </div>
        
        <div class="card-content">
          <p class="api-note">POST /api/simulation/create</p>
          <p class="description">Граф построен. Переходите к следующему шагу — настройке среды.</p>
          <button 
            class="action-btn" 
...
// 进入Настройка среды - 创建 simulation 并跳转
const handleEnterEnvSetup = async () => {
  ...
  try {
    ...
    alert('Ошибка создания симуляции: ' + err.message)
  } finally {
