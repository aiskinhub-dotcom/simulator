      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            <template v-if="graphData">
              ...
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} Связь</span>
              <span class="stat-divider">|</span>
            </template>
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? '退出全屏' : 'Fullscreen'">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-if="graphData" class="graph-view">
            ...
            <!-- Панель деталей узлов/рёбер -->
            <div v-if="selectedItem" class="detail-panel">
              <div class="detail-panel-header">
                <span class="detail-title">{{ selectedItem.type === 'node' ? 'Детали узла' : 'Relationship' }}</span>
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">ГрафDataЗагрузка...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">ОжиданиеГенерация онтологии</p>
            <p class="waiting-hint">生成完成后将自动开始BuildГраф</p>
          </div>
        ...
        <div class="process-content">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  上传文档后，LLM分析文档内容，自动生成适合舆论Симуляция的本体结构（实体类型 + Связь类型）
                </div>
              </div>
              
              <!-- Генерация онтологииProgress -->
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成的Связь类型 ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} 更多Связь...
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">ОжиданиеГенерация онтологии...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">接口说明</div>
                <div class="detail-content">
                  Based on generated ontology，将文档分块后调用 Zep API BuildKnowledgeГраф，Extract entities andСвязь
                </div>
              </div>
              
              <!-- Ожидание本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">ОжиданиеГенерация онтологии完成...</div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">Build结果</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">准备Enter下一步骤</div>
              </div>
          ...
          <div class="next-step-section" v-if="currentPhase >= 2">
            <button class="next-step-btn" @click="goToNextStep" :disabled="currentPhase < 2">
              EnterНастройка среды
              <span class="btn-arrow">→</span>
            </button>
          </div>
        </div>

        <!-- Информация о проектеPanel -->
...
const ontologyProgress = ref(null) // Генерация онтологииProgress
...
const statusText = computed(() => {
  if (error.value) return 'Сборка не удалась'
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = '项目Инициализация失败: ' + (err.message || 'Неизвестная ошибка')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = '加载项目失败: ' + (err.message || 'Неизвестная ошибка')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // 保存 task_id 用于раундов询
      const taskId = response.data.task_id
      
      // 启动ГрафDataраундов询（独立于任务状态раундов询）
      startGraphPolling()
    ...
    error.value = '启动Построение графа失败: ' + (err.message || 'Неизвестная ошибка')
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // 先获取Информация о проекте以获取 graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ ГрафПостроение завершено，正在加载完整Data...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // обновление прогресса显示为完成状态
        buildProgress.value = {
          progress: 100,
          message: 'Построение завершено，正在加载Граф...'
        }
        
        // 重新加载项目Data获取 graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 加载完整Граф:', projectResponse.data.graph_id)
            await loadGraph(projectResponse.data.graph_id)
            console.log('✅ Граф加载完成')
          }
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Построение графа失败: ' + (task.error || 'Неизвестная ошибка')
        buildProgress.value = null
      }
    }
  } catch (err) {
